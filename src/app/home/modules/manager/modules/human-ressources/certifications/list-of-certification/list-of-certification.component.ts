...
@Component({
  selector: 'wid-list-of-certification',
...
export class ListOfCertificationComponent  implements OnInit, OnChanges, OnDestroy {
  ...
  refData = { };
  featureAdd = '';
  tabFeatureAccess = [{ name: 'certification.show', feature: 'HR_REQUEST_CERTIF'},
    { name: 'certification.download', feature: 'SOURCING_CAND_FILE_ACCESS'},
    { name: 'certification.update', feature: 'SOURCING_CAND_FILE_ACCESS'},
    { name: 'certification.delete', feature: 'SOURCING_CAND_FILE_ACCESS'},
    { name: 'certification.archive', feature: 'SOURCING_CAND_FILE_ACCESS'}];
  ...
  async ngOnInit(): Promise<void> {
    ...
    if (this.collaboratorAction) {
      this.featureAdd = 'SOURCING_CAND_FILE_ACCESS';
      // this.featureAdd = 'HR_REQUEST_CERTIF';
    }
    await this.getConnectedUser();
    await  this.getCertificate(this.nbtItems.getValue(), 0);
  ...
  switchAction(rowAction: any) {
    switch (rowAction.actionType.name) {
      case ('certification.show'):  this.showCertificate(rowAction.data[0]);
        break;
      case('certification.delete'):  this.onStatusChange(rowAction.data[0]);
        break;
      case('certification.download'):  this.pdf(rowAction.data[0]);
        break;
      case('certification.archive'): this.activateCertification(rowAction.data[0]);
        break;
      case('update') :
  ...
  async pdf(certificate: any) {
    ...
    if (certificate && certificate.request_status === 'Confirmed' && certificate.signature ) {
      ...
      this.utilsService.openSnackBar('certification not confirmed or not confirmed yet', 'close');
