...
export class UsersListComponent implements OnInit {
  ...
  load(): void {
    ...
    this.getRefdata();
    res.forEach(element => {
    element['profileTypeDesc'] = this.refData['PROFILE_TYPE'].find((type) =>
    type.value === element['user_type']).viewValue;
    element['profTitlesDesc'] = this.refData['PROF_TITLES'].find((title) =>
  ...
  applyFilter(filterValue: string): void {
    ...
    this.usersList.filter(
      (res) => {
        if (
          res.UserKey['email_address'].toLowerCase().includes(filterValue.toLowerCase()) ||
          ...
          res.email_adress.toLowerCase().includes(filterValue.toLowerCase()) ||
  ...
  onChangeStatus(id: string, status: string) {
        this.profileService.userChangeStatus(id, status, this.credentials['email_address']).subscribe(
          (res) => {
            this.load();
          },
